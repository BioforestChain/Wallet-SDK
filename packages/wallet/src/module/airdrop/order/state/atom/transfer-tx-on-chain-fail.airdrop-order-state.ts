...
/**转移交易上链失败状态 */
@Injectable()
export class TransferTxOnChainFail_AirdropOrderState extends AirdropOrderFinallyState {
    constructor() {
        super(AIRDROP_ORDER_STATE_ID.TRANSFER_TX_ON_CHAIN_FAIL);
    }
    ...
    async onEnterState(orderObj: AirdropOrderObj): Promise<void> {
        ...
        Logger.debug(`<${orderType}> airdropType:${airdropType} orderId:${orderId} 在${chainName}链上，转移dp失败`);
